...
import { SummaryReportQuery, SummaryReportResult, FinancialInsightsResult } from './reports.types';
...
export class ReportsService {
  static async getSummaryReport(userId: string, query: SummaryReportQuery): Promise<SummaryReportResult> {
    const now = new Date();
    const queryMonth = query.month || now.getMonth() + 1;
    const queryYear = query.year || now.getFullYear();

    const startDate = new Date(queryYear, queryMonth - 1, 1, 0, 0, 0, 0);
    const endDate = new Date(queryYear, queryMonth, 0, 23, 59, 59, 999);

    const budget = await ReportsRepository.findUserBudget(userId);
    ...
    const [incomeAggregate, expenseAggregate, walletBalanceAggregate] = await Promise.all([
      ReportsRepository.aggregateIncome(userId, startDate, endDate),
      ReportsRepository.aggregateExpense(userId, startDate, endDate),
      ReportsRepository.aggregateWallets(userId),
    ]);
    ...
    const categoryExpenses = await ReportsRepository.groupCategoryExpenses(userId, startDate, endDate);
    const categoryIds = categoryExpenses.map((c) => c.categoryId);
    const categoriesInfo = await ReportsRepository.findCategoriesByIds(categoryIds, userId);
  ...
  static async getFinancialInsights(userId: string): Promise<FinancialInsightsResult> {
    const now = new Date();
    ...
    const [budget, currentTxs, prevTxs, goals, recurring] = await Promise.all([
      ReportsRepository.findUserBudget(userId),
      ReportsRepository.findPeriodTransactions(userId, curStart, curEnd),
      ReportsRepository.findPeriodTransactions(userId, prevStart, prevEnd),
      ReportsRepository.findUserGoals(userId),
      ReportsRepository.findActiveRecurring(userId),
    ]);
