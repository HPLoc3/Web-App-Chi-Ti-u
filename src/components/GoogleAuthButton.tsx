...
export const GoogleAuthButton: React.FC<GoogleAuthButtonProps> = ({
  ...
  useEffect(() => {
    // Phát hiện ứng dụng đang chạy trong iFrame (AI Studio preview / embedded)
  ...
  // Google OAuth Popup Fallback (Xử lý khi nút Google Iframe bị chặn trong iFrame/Webview hoặc popup)
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full space-y-2">
      <div className="w-full flex justify-center min-h-[40px]">
        {isIframe || showPopupFallback ? (
          renderCustomPopupButton(showPopupFallback ? 'Đăng nhập với Google (Popup)' : 'Tiếp tục với Google')
        ) : (
          <GoogleLogin
      ...
      {!isIframe && !showPopupFallback && (
        <button
          type="button"
          onClick={() => triggerGooglePopup()}
          className="text-[11px] text-stone-500 hover:text-emerald-700 underline transition cursor-pointer"
        >
          Gặp sự cố khi nhấn nút? Dùng cửa sổ Google Popup
        </button>
      )}

      {isLoading && (
        <p className="text-xs text-emerald-800 font-medium animate-pulse text-center flex items-center justify-center gap-1.5 mt-1">
