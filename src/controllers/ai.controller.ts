...
/**
 * Hàm loại bỏ các ký tự điều khiển nguy hiểm và sanitize nội dung user input
 */
 ...
 * Trợ lý tài chính thông minh tích hợp phòng thủ AI Prompt Injection
 */
export const aiFinancialAssistant = async (req: Request, res: Response): Promise<void> => {
  const { message, context } = req.body;

  const rawCleanMessage = sanitizePromptInput(message);
  ...
  // Prompt Engineering với Cơ chế Phân định Ranh giới Dữ liệu (Defensive Delimiter Tags)
  // Phòng chống Prompt Injection & Jailbreak Override
...
- reply: Câu trả lời thân thiện, lịch sự bằng tiếng Việt.`;
  ...
  try {
    ...
    Logger.error('Gemini AI Assistant Error:', error, req.id);
    res.status(200).json({
      success: false,
      fallbackToRule: true,
